...
class OperationController extends Controller
{
    ...
    public function retraitParDate(Request $req){
        ...
        $donnee = $this->operationInstance->getRetraitParDate($req);
    ...
    public function retraitParMois(Request $request)
    {
        // récupération du numéro de compte
        //$numeroCompte = $request->numCompte;
        ...
        $resultat = $this->operationInstance->retraitParMois($request);

        return \response()->json($resultat, 201)->withCallback($request->input('callback'));
    }

    public function retraitOperateurParMois(Request $request)
    {
        // récupération du numéro de compte
        //$numeroCompte = $request->numCompte;
        ...
        $resultat = $this->operationInstance->retraitOperateurParMois($request);

        return \response()->json($resultat, 201)->withCallback($request->input('callback'));
    }

    public function retraitParDateParOperateur(Request $request)
    {
        // récupération du numéro de compte
        //$numeroCompte = $request->numCompte;
        ...
        $resultat = $this->operationInstance->getRetraitParDateParOperateur($request);
