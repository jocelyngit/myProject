...
Ext.define('zonblewou.view.mesPages.operations.regularisation.CotisationFormController', {
    ...
    */
    ...
    onEnregRegButtonClick: function() {
        ...
        if (parseInt(p['montantCotisation']) < parseInt(p['mise']))
        {
            Ext.Msg.show({
                title: 'ERREUR',
                msg: 'Le montant de la cotisation ne doit pas être inférieur à la mise.',
                width: 250,
                buttons: Ext.Msg.OK,
                fn: function(btn) {
                    if (btn == 'ok') {
            ...
            var montantCotField = this.lookupReference('montantCotRegRef'),
                miseField = this.lookupReference('miseRegRef');
            ...
            form.getForm().getFields().items[7].setValue('');
